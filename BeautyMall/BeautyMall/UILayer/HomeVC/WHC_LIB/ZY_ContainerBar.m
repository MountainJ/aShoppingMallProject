...
#define KZY_TitleMargin   (10.0)     //标题文字边距
#define KZY_DropImageSize (12.0)     //下拉图片尺寸
#define KZY_AnimatedDuring (0.2)     //游标移动动画周期
@implementation ZY_ContainerBarParam
...
+ (ZY_ContainerBarParam *)getZY_ContainerViewParamWithTitles:(NSMutableArray*)titlesArr laterTitlesArr:(NSMutableArray *)laterTitlesArr{
    ZY_ContainerBarParam  * param = [ZY_ContainerBarParam new];
    if(titlesArr == nil){
    ...
    if(dict && dict.count > 0){
        NSMutableArray  * tempTitlesArr = [dict[KZY_ContainerTitlesArrKey] mutableCopy];
        NSMutableArray  * tempLaterTitlesArr = [dict[KZY_ContainerLaterTitlesArrKey] mutableCopy];
        ...
        if(tempLaterTitlesArr.count + tempTitlesArr.count < titlesArr.count + laterTitlesArr.count){
            ...
            [ZY_ContainerBar saveContainerBarTitlesArr:tempTitlesArr laterTitlesArr:tempLaterTitlesArr];
        }
...
@interface ZY_ContainerBar ()<ZY_ContainerBarItemDelegate>{
    ZY_ContainerBarParam             * _containerBarParam;          //容器参数
...
@implementation ZY_ContainerBar
...
- (void)updateContainerBarItemView{
    for (UIView * view in _containerBarItemView.subviews) {
        if([view isKindOfClass:[ZY_ContainerBarItem class]]){
    ...
    for (NSInteger i = 0; i < _barItemCount; i++) {
        ...
        ZY_ContainerBarItem  * barItem = [[ZY_ContainerBarItem alloc]initWithFrame:barItemRC];
...
- (void)initUILayout{
    ...
    containerBarItemViewRC.size.width = self.width - KZY_DropBtn_Width;
    ...
    _dropBtn.frame = CGRectMake(_containerBarItemView.maxX, dropBtnY, KZY_DropBtn_Width, _containerBarItemView.height);
...
- (UIImage *)getDropImage{
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(KZY_DropImageSize, KZY_DropImageSize), NO, 1.0);
    CGContextRef  context = UIGraphicsGetCurrentContext();
    CGContextSetLineWidth(context, 1.0);
    CGContextSetLineCap(context, kCGLineCapRound);
    CGContextSetStrokeColorWithColor(context, [UIColor blackColor].CGColor);
    CGContextMoveToPoint(context, 0.0, startPostion);
    CGContextAddLineToPoint(context, KZY_DropImageSize / 2.0, KZY_DropImageSize - startPostion);
    CGContextAddLineToPoint(context, KZY_DropImageSize, startPostion);
...
+ (void)saveContainerBarTitlesArr:(NSArray *)titlesArr laterTitlesArr:(NSArray *)laterTitlesArr{
    ...
    NSDictionary    * dict = @{KZY_ContainerTitlesArrKey:titlesArr,
                               KZY_ContainerLaterTitlesArrKey:laterTitlesArr};
    [ud setObject:dict forKey:KZY_ContainerConfigurationKey];
    [ud synchronize];
}
...
#pragma clang diagnostic pop
    ...
    return barItemWidth + KZY_TitleMargin;
}

- (void)clickDropBtn:(UIButton*)sender{
    sender.selected = !sender.selected;
    _dropImageView.transform = CGAffineTransformMakeRotation(M_PI);
    if(_delegate && [_delegate respondsToSelector:@selector(ZY_ContainerBar:clickDrop:)]){
        [_delegate ZY_ContainerBar:self clickDrop:sender];
    }
}
...
- (ZY_ContainerBarItem *)barItemWithIndex:(NSInteger)index{
    ZY_ContainerBarItem  * item = nil;
    for (ZY_ContainerBarItem * itemBar in _containerBarItemView.subviews) {
        if([itemBar isKindOfClass:[ZY_ContainerBarItem class]] && index == itemBar.index){
            item = itemBar;
            break;
        }
    }
...
- (void)resetBarItemStateMaxIndex:(NSUInteger)index oriX:(CGFloat)oriX{
    ...
    ZY:
        for (i = 0; i < count; i++) {
            ZY_ContainerBarItem * barItem = [self barItemWithIndex:i];
...
- (void)dynamicChangeStateOffsetX:(CGFloat)offsetX oriX:(CGFloat)oriX{
    if(YES){
        ...
        ZY_ContainerBarItem  * currentBarItem = [self barItemWithIndex:pageIndex - 1];
        ...
        ZY_ContainerBarItem  * unknownBarItem = [self barItemWithIndex:pageIndex];
...
- (void)updateContainerClickIndex:(NSInteger)index{
    [self updateContainerBarItemView];
    ZY_ContainerBarItem  * tempBarItem = nil;
    for (ZY_ContainerBarItem  * item in _containerBarItemView.subviews) {
        if([item isKindOfClass:[ZY_ContainerBarItem class]]){
            if(item.index == index){
                tempBarItem = item;
            }
        }
    }
    [self ZYContainerBarItem:tempBarItem clickIndex:index animated:YES];
}
...
- (void)ZYContainerBarItem:(ZY_ContainerBarItem*)barItem clickIndex:(NSInteger)index animated:(BOOL)animated{
    if(_delegate && [_delegate respondsToSelector:@selector(ZY_ContainerBar:clickIndex: animated:)]){
        [_delegate ZY_ContainerBar:self clickIndex:index animated:animated];
    }
    ...
    CGFloat  animatedDuring = KZY_AnimatedDuring;
    if(!animated){
        animatedDuring = 0.0;
    }
