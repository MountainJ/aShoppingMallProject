...
//    
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [imgView sd_setImageWithURL:[NSURL URLWithString:urlString] placeholderImage:[UIImage imageNamed:@"def_02"]];
    });
...
-(void)refreshUI
{
    ...
    for (int i = 0;i<_pageCount;i++) {
        ...
        imageView.frame = CGRectMake((self.frame.size.width * i) + 0 + AppScreenWidth, 0, self.frame.size.width, self.frame.size.height);
    ...
    _lastImageView.tag = _pageCount+500;///?+500
...
//    _pageControl.hidden =!showPageControl;///?jay
//    NSLog(@"%d",_pageControl.hidden);
}
