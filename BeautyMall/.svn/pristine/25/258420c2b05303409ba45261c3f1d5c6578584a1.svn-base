...
#define KZY_BadgeFontSize                     (14.0) // 标记字体大小
#define KZY_DefaultMaxCircleDistanceMultiple  (11.0) // 最大圆心距离倍数

@interface ZY_BadgeView ()<UIGestureRecognizerDelegate>{
...
- (instancetype)initWithSuperView:(UIView *)superView position:(CGPoint)position radius:(CGFloat)radius{
    ...
    if(self){
        ...
        _badgeLabel.font = [UIFont systemFontOfSize:KZY_BadgeFontSize];
...
- (void)initData{
    ...
    _maxCircleDistance = _radius * KZY_DefaultMaxCircleDistanceMultiple;
...
- (void)removeSelf{
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        ...
        if(_delegate && [_delegate respondsToSelector:@selector(ZY_BadgeViewDidRemoveFromSuperViewWithIndex:)]){
            [_delegate ZY_BadgeViewDidRemoveFromSuperViewWithIndex:_index];
        }
    ...
    switch (panGesture.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            if(_enableDragBadgeLabel){
                ...
                if(_delegate && [_delegate respondsToSelector:@selector(ZY_BadgeViewWillStartDrag)]){
                    [_delegate ZY_BadgeViewWillStartDrag];
                }
            }
        }
            break;
        ...
        case UIGestureRecognizerStateEnded:{
            ...
            if(_enableDragBadgeLabel){
                ...
                if(_delegate && [_delegate respondsToSelector:@selector(ZY_BadgeViewDidEndDrag)]){
                    [_delegate ZY_BadgeViewDidEndDrag];
                }
            }
        }
        default:
